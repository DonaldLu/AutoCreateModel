...
namespace AutoCreateModel
{
    ...
    public class CreateToilet : IExternalEventHandler
    {
        ...
        private List<FamilyInstance> CreateToiletFamilyInstance(Document doc)
        {
            ...
            // 男廁模型
            List<FamilyInstance> manDataModels = toiletModels.Where(x => x.Symbol.FamilyName.Equals("MRT_廁所群組(一般坐式)(SinoBIM-第2版)") ||
                                                                         x.Symbol.FamilyName.Equals("MRT_洗手台群組(SinoBIM-第3版)") ||
                                                                         x.Symbol.FamilyName.Equals("MRT_小便斗群組(SinoBIM-第1版)")).Distinct(new RemoveDuplicatesComparer()).ToList();
            ...
            using (Transaction trans = new Transaction(doc, "放置元件"))
            {
                ...
                foreach (ManData manData in jsonData.ManDataList)
                {
                    ...
                    foreach (FamilyInstance manDataModel in manDataModels)
                    {
                        ...
                        {
                            if (symbol.FamilyName.Equals("MRT_洗手台群組(SinoBIM-第3版)"))
                            {
                                xyz = new XYZ(xyz.X + 20, xyz.Y, xyz.Z);
                            }
                            else if (symbol.FamilyName.Equals("MRT_小便斗群組(SinoBIM-第1版)"))
                            {
                                xyz = new XYZ(xyz.X + 40, xyz.Y, xyz.Z);
                            }
                        }
                        FamilyInstance instance = doc.Create.NewFamilyInstance(xyz, symbol, StructuralType.NonStructural);
                        if (instance.Symbol.FamilyName.Equals("MRT_廁所群組(一般坐式)(SinoBIM-第2版)"))
                        {
                            Parameter counts = instance.LookupParameter("一般坐式數量");
                            counts.Set(manData.Toilet_Count);
                        }
                        else if (instance.Symbol.FamilyName.Equals("MRT_洗手台群組(SinoBIM-第3版)"))
                        {
                        ...
                        else if (instance.Symbol.FamilyName.Equals("MRT_小便斗群組(SinoBIM-第1版)"))
                        {
